...
data "local_file" "install_tools" {
    filename = "${path.module}/install_tools.yaml"
}
...
resource "aws_instance" "bastion" {
    ...
    provisioner "local-exec" {
        command = <<EOT
        ansible-playbook -vvv -i ${aws_instance.bastion.public_ip}, -u ec2-user --private-key=${var.key_name}.pem ${data.local_file.install_tools.filename} \
